...
func TestSimpleWriteOfSingleInjectionBatch(t *testing.T) {
	...
	for i := 0; i < 25; i++ {
		...
		injections[i] = model.Injection{model.Time{readTime.Unix(), "America/Los_Angeles"}, float32(i), "Levemir", "Basal"}
	}
...
func TestSimpleWriteOfInjectionBatches(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < 24; j++ {
			...
			injections[j] = model.Injection{model.Time{readTime.Unix(), "America/Los_Angeles"}, float32(1.5), "Levemir", "Basal"}
		}
